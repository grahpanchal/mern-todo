...
function TodoApp() {
  ...
  const handleTodoUpdated = useCallback((updated) => {
    setTodos((prev) => prev.map((t) => (t._id === updated._id ? updated : t)));
    showNotif("✏️ Todo update hua!");
  }, []);

  const handleTodoDeleted = useCallback(({ id }) => {
    setTodos((prev) => prev.filter((t) => t._id !== id));
    showNotif("🗑️ Todo delete hua!");
  }, []);

  // userId pass karo — user-wise room join hoga
  useSocket(user?.id, handleTodoAdded, handleTodoUpdated, handleTodoDeleted);
